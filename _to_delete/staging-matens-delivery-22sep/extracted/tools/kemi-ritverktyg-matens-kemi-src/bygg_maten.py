...
DETALJERAD_ALT2D = ('Haworth-projektion av glukosringen med svart text på vit bakgrund: alla fem '
    ...
    'väteatom (H, svart) och sin OH-grupp (röd) utsatta lodrätt uppåt eller nedåt – inga atomer är dolda '
    'eller underförstådda. C5 har dessutom grenen CH2OH (uppåt, OH i rött).')
...
def detailed_glucose_card_html():
    ...
    Bilden (glukos-detaljerad.png) är Jespers egen referensbild (Hämtade filer, 22 sep 2026), beskuren
    till bara titel+diagram. Ett tidigare försök att i stället RITA upp samma sak som egen SVG-kod (med
    korrekt lodrät Haworth-konvention, se historiken i OVERLAMNING.md) blev enligt Jesper inte snyggt
    nog ("ser inte bra ut") – han bad då uttryckligen om att antingen få en identisk kopia eller, om det
    inte gick, använda hans bild direkt. Bilden är beskuren så att bildtextens felaktiga "α-D-glukos"-
    etikett INTE följer med (se OVERLAMNING.md: det uppåt/nedåt-mönster bilden visar - OH uppåt på C1,
    samma sida som CH2OH på C5 - kännetecknar egentligen β-anomeren, inte α; kapitlet tar av avsikt inte
    upp alfa/beta-skillnaden alls, så bilden är här bara "glukos", utan anomer-namn)."""
    m = TEXTS['glukos']
    png = IMG / 'kulmodeller' / 'glukos.png'
    pw, ph = _dims(png)
    # OBS: _dims() delar med 3 (kulmodeller renderas i 3x för skärmskärpa) - den här bilden är i stället
    # sparad i 2x (620x496 för ~310x248 visning), samma faktor som ölglasbilden i studieguiden.
    from PIL import Image as _PILImage
    _im = _PILImage.open(IMG / 'strukturformler' / 'glukos-detaljerad.png')
    ...
    struct = (f'<div><span class="mc-lab">Strukturformel (detaljerad)</span><div class="mc-struct">'
              f'<img src="/images/kemi/matens-kemi/strukturformler/glukos-detaljerad.png" width="{sw}" height="{sh}" '
              f'alt="{alt2d}" loading="lazy"></div></div>')
