...
- **Tillägg 22 sep 2026 (kväll) – glukos-detaljerad.png ersatte glukos-detaljerad.svg:** Jesper bad
  om att lära sig Haworth-konventionen (H/OH som strikt vertikala streck upp eller ner per
  kolatom, inte utåtriktade i hexagonens egen vinkel) från sin referensbild `Glukos.png` och att
  bytta den ritade SVG:n. Första försök: skrev om `ring_glucose_detailed_svg()` i `bygg_maten.py`
  med korrekt geometri (bytte hexagon-orientering från "spetsig topp" till "platt topp" – annars
  pekade de vertikala bindningarna vid syre/C5 IN i ringen; verifierat med inzoomad skärmdump).
  Jesper: "Den strukturformel som nu ligger i Glukos - detaljerad formel, ser inte bra ut" – bad om
  antingen en exakt likadan bild som referensen, eller referensbilden själv om det första inte gick.
  **Beslut: använd Jespers egen bild.** `ring_glucose_detailed_svg()` och dess post i
  `FREESTANDING_SVG`-dicten borttagna helt ur `bygg_maten.py`. Ny fil
  `images/kemi/matens-kemi/strukturformler/glukos-detaljerad.png` (620×496, ingen EXIF) – beskuren
  ur `Glukos.png` (Jespers Hämtade filer) så att bara rubriken "Glukos / C₆H₁₂O₆" och själva
  Haworth-ritningen syns. `detailed_glucose_card_html()` skriven om att referera PNG:n direkt, med
  egen PIL-skalning (bredd/höjd /2 – INTE `_dims()`-hjälparen, som antar /3-konventionen för
  kulmodellsbilderna).
  **Kemifynd, flaggat men INTE ändrat i Jespers bild:** bildtexten i originalet säger
  "α-D-glukos (Haworth-projektion)", men det ritade mönstret (OH upp C1/C3, OH ner C2/C4, CH₂OH upp
  C5) motsvarar egentligen **β-D-glukopyranos** (vanligast i lösning, ~64 %) – vid α ska C1-OH peka
  NER. Löst genom att beskära bort just den bildtextraden ur den använda bilden (bara rubriken och
  ritningen kvar, inget alfa/beta-påstående), snarare än att antingen sprida vidare eller själv
  omdefiniera Jespers bild utan att fråga honom. Flaggat i meddelandet till Jesper; kan läggas till
  en korrekt bildtext senare om han vill.
  Omtestat: Playwright 1280/390 px (bilden + ölglasbilden får plats i `.m-side` utan overflow),
  axe-core samma enda kända avvikelse som förut.
- **Tillägg 22 sep 2026 (kväll) – färgschema omgjort (mindre gult):** se `CLAUDE.md`,
  avsnittet "Tillägg 22 sep 2026 (kväll) – färgschema omgjort, mindre gult" för full motivering
  (CVD-simulering, vilka färger som bytte och varför, vad som lämnades orört). Kort sammanfattat:
  hela kapitlets AREA-varumärkesfärg (orange) bytt till vinrött/fuchsia i alla sju byggfiler som
  använder den; `.deepen`s gulbeige färg bytt till neutral gråblå ENDAST i denna mapps egen
  `src/studieguide.css` (påverkar inte andra kapitel); `.viktigt` och `.check-q` oförändrade.
