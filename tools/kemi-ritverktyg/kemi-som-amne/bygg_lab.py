#!/usr/bin/env python3
"""Genererar kemi/kemi-som-amne/laborationer.html från labdata.py (utskrivbara protokoll som HTML-sida)."""
import os, sys, html
...
import re as _re
def wrap_tables(h):
    """Lägger varje resultattabell i en fokuserbar, rullbar ruta (tillgängligt på smal skärm)."""
    return _re.sub(r'(<table class="resultat-table">.*?</table>)',
                   lambda m: '<div class="tbl-scroll" tabindex="0" role="region" aria-label="Tabell (rulla i sidled på smal skärm)">' + m.group(1) + '</div>', h, flags=_re.S)
...
def lab_html(n, L):
    ...
    if L.get("plan_text"):
        plan = '<h3>Din plan</h3><div class="plan-box"><strong>%s</strong><span class="svarslinje"></span><span class="svarslinje"></span><span class="svarslinje"></span></div>' % L["plan_text"]
    graph = ""
    if L.get("graf"):
        graph = '<h3>%s</h3><div class="grafruta" role="img" aria-label="%s"></div>' % L["graf"]
...
CSS = '''
    ...
    @media print {
      ...
      .svarslinje { height: 1.4rem; }
      .resultat-table td { height: 1.9rem; }
...
'''
    open(os.path.join(ROOT, "kemi", "kemi-som-amne", "laborationer.html"), "w", encoding="utf-8").write(out)
