...
**Regeländring efter Jespers feedback (22 sep 2026):** bytte poängmodell från "första lag som svarar rätt" till lagkonsensus (se Spelregler ovan) – motorn (`js/engine.js`) omskriven, rankning sker på lagets "alla klara + alla rätt"-tidpunkt. **Öppen fråga:** vid exakt samtida klart-tidpunkt (samma millisekund) avgörs turordningen av en global, monotont stigande svars-sekvensräknare (deterministisk, inte slumpad) – Jesper bör bekräfta att detta är rimligt eller ange en annan tie-break-regel.

**Ny funktion – "Slumpa lag" (22 sep 2026):** ny modul `js/slumpa-lag.js` för automatisk lagindelning (se ovan). **Öppen fråga:** vad ska hända om en elev ansluter EFTER att slumpningen redan gjorts och matchen ev. redan startat – nu hamnar de i "Utan lag"-poolen precis som i manuella läget, men Jesper bör bekräfta att det är önskat beteende även i slumpat läge.

**Osäkert/kvarstår (oförändrat sedan ursprungsbygget):** själva `wrangler login`/`deploy` mot Jespers Cloudflare-konto är overifierat (allt annat testat mot riktig `workerd`-körtid lokalt); nätverksrättvisa vid skolans riktiga uppkoppling otestad (allt kört lokalt med mikrosekunders latens); fem frågor med lite svagare källor flaggade i `tools/hastkapplopning-worker/OVERLAMNING.md` (bl.a. M3 fråga 4 om kosttillskott, M2 fråga 3 källan bara "Läkartidningen") – läs igenom frågorna i `spel/hastkapplopning/data/mineraler-och-vitaminer.json` innan lektion. **Facit-sekretess vid lagkonsensus** bör dubbelkollas av Jesper: designen är fortfarande att läraren skickar facit per fråga (aldrig i förväg), men eftersom ett lag nu måste vänta på sin sista medlem finns en teoretisk risk att en snabb elev kan hinna säga svaret högt till lagkompisen innan alla svarat – detta är en pedagogisk/klassrumsfråga, inte en teknisk läcka, men värd att känna till.

**Testat om (efter regeländringen):** enhetstester motorn 35/35, integrationstest mot riktig `wrangler dev` 6/6, persistenstest 10/10, E2E Playwright hela 3-matcherspelet (Lokalt läge + riktig Worker, inkl. Slumpa-lag-flödet) 980/980 kontroller, axe-core 30 sidtillstånd 0 överträdelser.
