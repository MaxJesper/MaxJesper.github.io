...
- **Etiketter i rullistan (Jesper, 20 sep 2026):** "🇸🇪 Svenska" och "🇬🇧 English" utan tillägg (engelska = HELA siten på engelska när den är översatt – tills dess ändras bara begreppen). Övriga språk: svenska först, sedan samma text på språket självt, t.ex. "🇸🇦 Arabiska (begrepp och checklistor) – العربية (المفاهيم وقوائم التحقق)". Byggs i `js/concept-lang-selector.js` (`STOD`, `ENGELSK_HELA`). Etiketten lovar checklistor → **checklistorna måste översättas till alla 11 språk och samma väljare läggas på checklistesidorna** (öppen uppgift, ej gjort 20 sep). Väljarens etikett: "🌐 Språk / Language:". Översättningarna av "begrepp"/"begrepp och checklistor" i etiketterna är AI-genererade – låt modersmålstalare titta på dem.
