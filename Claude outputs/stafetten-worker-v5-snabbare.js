 ...
 * ======================================================================================
 *
 * ============================== TURORDNING INOM LAGET ================================
 * Spelet slumpar fram vem i laget som får svara – ingen medspelare väljer (skrotad idé:
 * gav en elev risken att kännas utpekad, eller alltid bli vald sist). Varje lag har en
 * egen turordning (array med spelarnamn) och ett turIndex; turNu = turordning[turIndex]
 * är den enda som får lämna ett giltigt svar just nu (kollas i /svara). Ordningen avancerar
 * ett steg per fråga i /sammanstall (den enda skrivpunkten till delat tillstånd, se ovan) –
 * alltså EFTER att alla lag hunnit svara, aldrig i /svara, så det introducerar ingen ny
 * krockrisk. När turIndex når slutet av arrayen har alla i laget svarat lika många gånger;
 * då slumpas en helt ny ordning fram för nästa varv. Nya medlemmar som ansluter mitt i ett
 * pågående varv läggs sist i den aktuella ordningen och blir en del av nästa varvs slumpning.
 * ======================================================================================
 *
 * Endpoints:
 *   POST /rum                              -> {kod}
...
function nyttLag() {
  // medlemmar/turordning/turIndex/turNu styr VEM i laget som får svara på nästa fråga
  // (se "TURORDNING" i filhuvudet) – ingen individuell poäng, bara vem som får klicka.
  return { poang: 0, sparatStold: 0, senasteResultat: null, medlemmar: [], turordning: [], turIndex: 0, turNu: null };
}
// Fisher-Yates. Används för att slumpa fram en helt ny turordning när ett varv är klart.
function slumpaOrdning(arr) {
  const a = [...arr];
...
export default {
  async fetch(request, env) {
    ...
    // POST /rum/:kod/gaMed – gå med i (eller skapa) ett lag, med sitt eget namn
    if (delar.length === 3 && delar[2] === "gaMed" && request.method === "POST") {
      ...
      if (typeof body.spelare !== "string" || !body.spelare.trim()) return json(400, { fel: "ogiltigt spelarnamn" });
      const lag = body.lag.trim().slice(0, 24);
      const spelare = body.spelare.trim().slice(0, 24);
      const state = await hamtaRum(env, kod);
      if (!state) return json(404, { fel: "rummet finns inte" });
      if (!state.lag[lag]) state.lag[lag] = nyttLag();
      const team = state.lag[lag];
      if (!team.medlemmar.includes(spelare)) {
        team.medlemmar.push(spelare);
        // Läggs sist i pågående varv (se TURORDNING i filhuvudet) – slumpas fritt in i nästa.
        team.turordning.push(spelare);
        if (team.turNu === null) team.turNu = team.turordning[team.turIndex];
    ...
    if (delar.length === 3 && delar[2] === "startaFraga" && request.method === "POST") {
      ...
      // svarstid (sekunder) är läraren-konfigurerad och skickas med varje fråga; 45s
      // om inget annat anges (klienten skickar alltid ett värde numera, det här är bara
      // en reserv). Deadline räknas ut en gång här (server-tid) så att klienter aldrig
      // behöver lita på sin egen klocka.
      const svarstid = Number(body.svarstid) > 0 ? Number(body.svarstid) : 45;
      const startad = Date.now();
      state.aktuellFraga = {
        id: body.fragaId,
        typ: body.typ,
        text: typeof body.text === "string" ? body.text : "",
        alternativ: Array.isArray(body.alternativ) ? body.alternativ : [], // se kommentar i elevsidan om formatet
        flerval: !!body.flerval,
        rattSvar: body.rattSvar,
        svarstid,
        startad,
        deadline: startad + svarstid * 1000,
        sammanstalld: false,
    ...
    if (delar.length === 3 && delar[2] === "svara" && request.method === "POST") {
      ...
      const team = state.lag[body.lag];
      if (!team) return json(400, { fel: "okänt lag" });
      // Bara den vars tur det är får svara för laget (se TURORDNING i filhuvudet). Om
      // turNu av någon anledning saknas (t.ex. gammalt lag utan medlemmar) släpps alla
      // igenom, hellre än att låsa laget helt.
      if (team.turNu && body.spelare !== team.turNu) {
        return json(403, { fel: "inte din tur att svara", turNu: team.turNu });
      }
      if (state.aktuellFraga.deadline && Date.now() > state.aktuellFraga.deadline) {
        return json(403, { fel: "tiden är ute" });
      }
      ...
      await env[KV].put(nyckel, JSON.stringify({ svar: body.svar, tid: Date.now(), spelare: body.spelare || null }), { expirationTtl: RUM_TTL_SEKUNDER });
    ...
    if (delar.length === 3 && delar[2] === "sammanstall" && request.method === "POST") {
      ...
      // VIKTIGT FÖR HASTIGHETEN: hämta alla lags svar SAMTIDIGT (Promise.all), inte ett i
      // taget. Varje KV-anrop har en egen nätverksrundtur (kan lätt vara flera hundra ms),
      // och med en sekventiell loop lades de rundturerna på hög – med t.ex. 5 lag blev
      // "Sammanställ" flera sekunder långsammare än den behövde vara. Parallellt tar hela
      // steget ungefär lika lång tid som EN rundtur, oavsett hur många lag som svarat.
      const rader = await Promise.all(lista.keys.map(k => env[KV].get(k.name)));
      const svar = [];
      lista.keys.forEach((k, i) => {
        const rad = rader[i];
        if (!rad) return;
      ...
      // Turordning: stafettpinnen går vidare till nästa spelare i varje lag – oavsett om
      // laget svarade rätt, fel eller inte alls den här frågan (se TURORDNING i filhuvudet).
      // Detta är den enda platsen som flyttar turen, så det introducerar ingen ny krockrisk.
      for (const lag of Object.keys(state.lag)) {
        const team = state.lag[lag];
        if (!team.turordning || team.turordning.length === 0) continue;
        team.turIndex += 1;
        if (team.turIndex >= team.turordning.length) {
          team.turIndex = 0;
          team.turordning = slumpaOrdning(team.turordning);
        }
        team.turNu = team.turordning[team.turIndex];
      }
