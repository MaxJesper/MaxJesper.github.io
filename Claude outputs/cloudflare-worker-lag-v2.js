 ...
 *                                       gåvomeddelande istället för det vanliga.
 */
...
async function hanteraForfragan(request, env) {
    ...
    if (delar.length === 3 && delar[2] === "lagPoangbyte" && request.method === "POST") {
      ...
      const riktning = body && body.riktning === "ge" ? "ge" : "ta";
      if (
      ...
      // Vid en GÅVA loggas ett slumpat, skämtsamt meddelande istället för det vanliga
      // "tog poäng från"-meddelandet – samma poängöverföring under huven, bara annan ton.
      const GAVOMEDDELANDEN = [
        (g, t, p) => `🎁 Lag ${g} vill demonstrera sin utomordentliga godhet och generositet genom att skänka ${p} poäng till lag ${t}. Ett tack vore på sin plats!`,
        (g, t, p) => `🎁 Lag ${g} vill fira lag ${t} med en liten present – ${p} poäng!`,
        (g, t, p) => `🎁 Eftersom lag ${g} är så framgångsrikt vill det gärna ge lite fickpengar till lag ${t}: ${p} poäng!`,
        (g, t, p) => `🎁 Till er alla i lag ${t} från oss alla i lag ${g} önskar vi en riktigt god match och ger er denna gåva om ${p} poäng.`,
        (g, t, p) => `🎁 En liten tröst om ${p} poäng till er i lag ${t} från oss i lag ${g}! För vad är väl vänner värda som inte kan avstå smulor från ett av överflöd dignande bord?`,
        (g, t, p) => `🎁 För att ni i lag ${t} ska inse vår godhet får ni en gåva om ${p} poäng, från lag ${g}.`,
        (g, t, p) => `🎁 Se nu till att vinna, lag ${t} – här är en gåva om ${p} poäng från lag ${g}.`,
      ];
      const gavoText = GAVOMEDDELANDEN[Math.floor(Math.random() * GAVOMEDDELANDEN.length)](gerNamn, tarNamn, faktiskPoang);
      ...
      loggaHandelse(state, {
        ...
        text: riktning === "ge"
          ? gavoText
          : `🔄 ${tarNamn} tog ${faktiskPoang} poäng från ${gerNamn}${etikett ? ` (${etikett})` : ""}`,
      });
      await sparaRum(env, kod, state);
