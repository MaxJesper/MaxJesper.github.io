...
- **Nytt beslut 22 sep 2026 (kväll) – matens-kemis varumärkesfärg bytt från orange till vinrött/fuchsia (bara detta kapitel):** Jesper tyckte sidan hade för mycket gult ("blek och tråkigt"). Utredning (CVD-simulering) visade att orsaken var att kapitlets HELA varumärkesfärg (den orangea "AREA"-färgen, använd i studieguide, laborationer, begreppskort/bingo, korsord, övningsverktyg och index – sju byggfiler) såg nästan likadan ut som `.deepen`-fördjupningsrutans gulbeige och den gula `.viktigt`-rutan vid simulerad röd-grön-färgblindhet. Bytt till vinrött/fuchsia (`#86198f` m.fl.) i alla sju filer i `tools/kemi-ritverktyg/matens-kemi/`. `.deepen`s färg bytt till neutral gråblå ENDAST i denna mapps egen `src/studieguide.css` – rör INTE den delade `.deepen`-klassen eller andra kapitels egna kopior (kemi-som-ämne, separationsprocesser har fortfarande sin gulbeige `.deepen`, oförändrat). Se CLAUDE.md och OVERLAMNING.md för full färgmotivering/CVD-siffror.
