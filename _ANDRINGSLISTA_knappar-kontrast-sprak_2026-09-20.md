...
- Piltangentsbläddring i instuderingsfrågor: `js/render-instudering.js` – sidan rullar så att nästa fråga hamnar på samma plats som förra (gäller alla områdens instuderingsfrågor).
- Instuderingsfrågor kemi: formelhjälp i textrutorna (`js/render-instudering.js`, `js/kemi-inmatning.js`), ritnotering på skärmen + ritutrymme vid utskrift (`css/instudering.css`; atomer fråga 15 och 20, periodiska-systemet fråga 11), fråga 21 i atomer omformulerad (tredimensionell form).
- Ett gemensamt språkval: gamla "Språk"-väljaren (uppläsningsröst) borttagen på alla 35 sidor och ersatt av "Begrepp översätts till" (index, Begreppslista, studieguide delar samma val). `js/lyssna.js` läser alltid upp på svenska. Berör alla `index.html`, `begreppslista.html` och `studieguide.html` i de 12 områdena samt `js/concept-lang-selector.js`.
- Språkväljarens etiketter: svenska först, sedan samma text på språket självt (t.ex. "Arabiska (begrepp) – العربية (المفاهيم)"), etikett "Språk / Language". `js/concept-lang-selector.js` (flaggorna `STOD` och `ENGELSK_HELA` styr texten när checklistor/engelska finns).
- Språkväljaren: tillägget står som "(begrepp)" tills checklistorna är översatta och korrekturlästa; då ändras det till "(begrepp och checklistor)" (flaggan `STOD`), och engelska får inget tillägg när hela siten är översatt (`ENGELSK_HELA`). Se påminnelse i `_CHECKLISTA_omraden.md`.

## 4. Nytt område: Separationsprocesser (20 sep 2026)

Hela området `kemi/separationsprocesser/` är nybyggt (tidigare bara en tom platshållare). Bygger på Jespers underlag (Gleerups kemibok, Enkel kemi) men med helt egen text, egna bilder och egna övningar. Sidor som behöver granskas: `index`, `studieguide` (9 milstolpar), `checklista`, `instuderingsfragor` (+print-elev/-larare), `ovningsprov` (+print), `facit` (+print), `begreppslista`, `begreppskort`, `larande-spel`, `ovningsverktyg`, `valj-metod`, `partiklar`, `laborationer`, `begrepp-bingo`, `korsord`. Nya delade/kringfiler: `kemi/separationsprocesser/js/partikelmodell.js`, 22 SVG i `images/kemi/separationsprocesser/`, byggverktyg i `tools/kemi-ritverktyg/separationsprocesser/`. Övningarna om aggregationsformer/grundämne–förening–blandning är anpassade från Atomer (Atomer är oförändrat). Alla knappar följer knappstandarden och alla sidor är testade mot WCAG 2.2 AA (axe).

## 7. Checklistor på arabiska, amhariska och kinyarwanda + fråga 10 i Atomer (20 sep 2026)

- **Checklistor översatta** till arabiska, amhariska och kinyarwanda för alla 15 områden: NYA filer `data/checklista.ar.json`, `checklista.am.json`, `checklista.rw.json` i `biologi/{genetik,hjarta-blod-lungor,liv-och-cellen}`, `fysik/{arbete-energi-effekt,elektricitet,kraft-och-rorelse,magnetism-induktion,materia,universum}`, `kemi/{atomer,elektrokemi,kol-och-kolforeningar,periodiska-systemet,separationsprocesser,syror-och-baser}` (45 filer). AI-översatta och maskinvaliderade (struktur, antal punkter, skrift, siffror/enheter/milstolpekoder), ej korrekturlästa.
- `js/render-checklista.js` – omskriven: visar översättningen under den svenska texten när språket valts (arabiska högerställd), bevarar ikryssade rutor vid språkbyte, tvåspråkiga knappar/rubrik, AI-märkning, hanterar även Universums JSON-format (fix: sidan visade tidigare "Inget innehåll hittades i checklistan").
- `js/concept-lang-selector.js` – skickar `conceptLangChange` vid språkbyte; arabiska, amhariska och kinyarwanda får etiketten "(begrepp och checklistor)" (flaggor `CHECKLISTA_SPRAK`, `VISA_CHECKLISTETIKETT`), övriga oförändrat "(begrepp)".
- `css/checklista.css` – stil för översättningsraderna (kontrast AA, RTL) och mörk rubrik vid utskrift (tidigare vit text = osynlig utan bakgrundsbild).
